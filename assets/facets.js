...
class FacetFiltersForm extends HTMLElement {
  ...
  static activeInStock() {
    const inputSizeFilter = Array.from(document.querySelectorAll('.input__size-filter'))
    const inputAvailability = document.querySelector('.input__availability-filter')

    inputSizeFilter.forEach((element) => {
      element.addEventListener('click', (event) => {
        inputAvailability.checked = true
      })
    })
  }
  ...
  static toggleSize() {
    ...
    const parmSize = urlParams.getAll('filter.v.option.size');
    const parmAvailable = urlParams.get('filter.v.availability');

    inputAvailability.checked = parmAvailable == '1' ?  true : false

    inputSizeFilter.map((elem) => {
      if(!parmSize.length) {
        elem.firstElementChild.checked = false
        return
      }

      parmSize.forEach((parm) => {
        elem.firstElementChild.checked = elem.firstElementChild.value == parm ? true : false
      })
    })
  }
...
FacetFiltersForm.activeInStock()
