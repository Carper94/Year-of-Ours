if (!customElements.get('variant-selects')) {
  ...
  class VariantSelects extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.updateMasterId();
      this.setDisabled();
      this.updateVariantAlternative()
    }

    updateVariantAlternative() {
      let filterByColor =
      this.options.length < 3
          ? this.variantData.filter((variant) =>
              variant.options.includes(this.options[1])
            )
          : this.variantData.filter(
              (variant) =>
                variant.options.includes(this.options[1]) &&
                variant.options.includes(this.options[2])
            );

      this.fieldsets.forEach((fieldset, i) => {
        if (fieldset.querySelectorAll('input').length) {
          fieldset.querySelectorAll('input').forEach((input, input_i) => {
            if(input.name == 'Size'){
              let findSizeByOptions = filterByColor.find((opt) => opt.options.includes(input.value))
              if(findSizeByOptions) {
                input.classList.toggle('is-disabled', !findSizeByOptions.available);
              } else {
                input.classList.toggle('is-disabled', true);
              }
            }
          });
        }
    ...
    setUnavailable() {
      ...
      submitButtons.forEach((submitButton) => {
        ...
        submitButtonText.textContent = 'Join the Waitlist';
